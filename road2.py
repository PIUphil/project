...
from PyQt5.QtCore import QCoreApplication, Qt, noshowbase
...
class RoadmapWindow(QMainWindow):
    ...
    def init(self):
        ...
        # 안내버튼
        startBt = QPushButton("안내시작 墳",self)
        startBt.move(620,600)
        startBt.resize(500,100)
        startBt.clicked.connect(self.onGuideBtClicked)
        startBt.setStyleSheet("background:#F06292;")
        startBt.setFont(QFont('MesloLGS NF', 45))
    ...
    def onInfoBtClicked(self):
        ...
        self.loadRoad()
        #self.label_pic.setPixmap(QPixmap("pic/headdress.png"))     #information
        #self.label_pic.setAlignment(Qt.AlignCenter)
        self.raiseBt()
    ...
    def onGuideBtClicked(self):
        ...
        step = tuple(np.round(((self.destination[0]-self.now[0])/7, (self.destination[1]-self.now[1])/7), 3))
        print(step)
